...
Class Action {
	...
	function login(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM borrowers where username = '".$username."' and password = '".$password."' ");
		if($qry->num_rows > 0){
	...
	function login2(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM borrowers where username = '".$email."' and password = '".md5($password)."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'passwors' && !is_numeric($key))
					$_SESSION['login_'.$key] = $value;
	...
	function logout(){
		...
		header("location:/debtor/login.php");
	}
	function logout2(){
		session_destroy();
		foreach ($_SESSION as $key => $value) {
